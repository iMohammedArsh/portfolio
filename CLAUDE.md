...
**Known limitation**: this is a pure client-rendered SPA with no server-side rendering. Traditional search crawlers generally handle CSR reasonably well today, but crawlers that don't execute JavaScript (including some AI/GEO crawlers) only see the static `index.html` shell plus `llms.txt` — they will not see section content, Projects data, etc. The real fix is a build-time prerendering step (a headless browser renders `dist/index.html` after `vite build` and overwrites it with static HTML); this has been deliberately deferred rather than bundled into unrelated UI work, and should be picked up as its own focused task.
